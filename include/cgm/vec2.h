 ...
 *
 */
CGMINLINE vec2 gmVec2Mscale(vec2 v, float s)
{
    float x = v.x * s;
    float y = v.y * s;
    return gmVec2(x, y);
 ...
 *
 */
CGMINLINE vec2 gmVec2Dscale(vec2 v, float s)
{
    float vx = v.x / s;
    float vy = v.y / s;
    return gmVec2(vx, vy);
 ...
 * @param n normal
 * 
 * @note if `CGM_ASSUME_NORMALIZED` is defined,
...
CGMINLINE vec2 gmVec2reflect(vec2 v, vec2 n)
{
    ...
    return gmVec2sub(v, gmVec2Mscale(n, 2.0f * d));
}
...
{
    return gmVec2sub(v, gmVec2floor(v));
}
...
{
    return gmVec2max(min, gmVec2min(v, max));
}
